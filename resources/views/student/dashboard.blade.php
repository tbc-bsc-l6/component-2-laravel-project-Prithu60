        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white shadow rounded p-6">
                ...
                @if($enrolledModules->count() === 0)
                    <p class="text-gray-600">You are not enrolled in any module right now.</p>
                @else
                    <ul class="space-y-3">
                            ...
                            <li class="flex items-center justify-between border rounded px-4 py-3">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600">
                                        Enrolled: {{ optional($module->pivot->enrolled_at)->format('Y-m-d') }}
                                    </p>
                                </div>
            ...
            <div class="bg-white shadow rounded p-6">
                ...
                @if($availableModules->count() === 0)
                    <p class="text-gray-600">No modules are currently available.</p>
                @else
                    <ul class="space-y-3">
                            ...
                            <li class="flex items-center justify-between border rounded px-4 py-3">
                                <div class="pr-4">
                                    ...
                                    <p class="text-xs text-gray-500 mt-1">
                                        Capacity: {{ $module->enrolledStudentsCount() }}/10
                                    </p>
                                </div>
                                ...
                                @if($atLimit)
                                    <button disabled class="px-4 py-2 rounded bg-gray-200 text-gray-600 cursor-not-allowed">
                                        Max 4 reached
                                    </button>
                                @elseif($isFull)
                                    <button disabled class="px-4 py-2 rounded bg-gray-200 text-gray-600 cursor-not-allowed">
            ...
            {{-- 3) Completed Modules --}}
            <div class="bg-white shadow rounded p-6">
                <h3 class="text-lg font-bold mb-4">Completed Modules (PASS / FAIL)</h3>
                        ...
                        @foreach($completedModules as $module)
                            <li class="flex items-center justify-between border rounded px-4 py-3">
                                <div>
                                    <p class="font-semibold">{{ $module->name }}</p>
                                    <p class="text-sm text-gray-600">
                                        Completed: {{ optional($module->pivot->completed_at)->format('Y-m-d') ?? '—' }}
                                    </p>
                                </div>

                                @if($module->pivot->status === 'PASS')
                                    <span class="px-3 py-1 rounded text-sm bg-green-100 text-green-800">PASS</span>
                                @else
                                    <span class="px-3 py-1 rounded text-sm bg-red-100 text-red-800">FAIL</span>
                                @endif
                            </li>
                        @endforeach
                    </ul>
